...
export const Hero: React.FC = () => {
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center text-center px-4 overflow-hidden pt-20">
      {/* Background elements */}
      <div className="absolute top-0 left-0 w-full h-full -z-10 bg-[radial-gradient(circle_at_center,_rgba(30,58,138,0.15)_0%,_rgba(5,5,5,1)_70%)]"></div>
      <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-[#d4af37]/5 blur-[120px] rounded-full -z-10 animate-pulse"></div>
      
      <div className="max-w-4xl mx-auto space-y-8">
        <div className="inline-block px-4 py-1 border border-[#d4af37]/30 rounded-full bg-black/50 text-[#d4af37] text-xs uppercase tracking-[0.3em] mb-4">
          La verdad ha sido fragmentada
        </div>
        
        <h1 className="text-5xl md:text-8xl font-mystery leading-tight">
          La realidad es el velo. <br />
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#d4af37] to-white italic">
      ...
      <div className="absolute bottom-10 animate-bounce cursor-pointer text-[#d4af37]/50" onClick={() => document.getElementById('about')?.scrollIntoView({ behavior: 'smooth' })}>
        <ChevronDown size={32} />
      </div>
    </section>
  );
